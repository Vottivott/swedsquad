...
#name = "projection_squad_train"
name = "projection_squad_ext_2_train"
name = "projection_squad_ext_2_dev"
name = "projection_squad_dev"

last_round = False

name = "dev_ext_2_multialt"
...
#for mode in ['read', 'write']:
    if mode == 'write':
        with open("translated dev answers proj-%d.txt" % proj_extend_n_words, encoding="utf-8") as f:
                    ...
                    if translate_answers:
                        for a_i, ans in enumerate(qa['answers']):
                            if mode == 'write':
                                ...
                                assert qa['answers'][a_i]['text'] in to_translate[t_i]
                                #qa['answers'][a_i][''] = translated[t_i]#.text
                                #if a_i == 0:
                                #    qas[q_i]['question'] += " [SEP] " + translated[t_i]
                                if not last_round:
                                    alt_qa = {}
                                    alt_qa['question'] = qas[q_i]['question'] + " [SEP] " + translated[t_i]
                                    alt_qa['answers'] = [ans]
                                    alt_qa['id'] = qas[q_i]['id'] + '_%d' % a_i
                                    alt_qas.append(alt_qa)
                                else:
                                    alt_qas[alt_qa_index]['question'] += " [SEP] " + translated[t_i]
                                    alt_qa_index += 1
                                t_i += 1
                            else:
                                a = qa['answers'][a_i]
                                s,e = (a['answer_start'], a['answer_start'] + len(a['text']))
                                c = context
                                pre = c[:s]
                                word = c[s:e]
                                post = c[e:]
                                pre_words = pre.split(' ')
                                pre_n = min(len(pre_words), proj_extend_n_words+1)
                                post_words = post.split(' ')
                                post_n = min(len(post_words), proj_extend_n_words+1)
                                extended = ' '.join(pre_words[-pre_n:]) + word + ' '.join(post_words[:post_n])
                                to_translate.append(extended)
                                #assert word == a['text']
                                #print(a['text'])
                                #print(extended)
                                #print(context)
    ...
    with open("to_translate_proj_%d.txt"%proj_extend_n_words,"w", encoding="utf-8") as out:
