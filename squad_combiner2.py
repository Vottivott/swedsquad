...
with open("translated train ot.json","r", encoding="utf-8") as f:
    data_ot = json.loads(f.read())

ot_qa = {}
for i,article in enumerate(data_ot['data']):
                    ...
                    for a_i, ans in enumerate(qa['answers']):
                        if only_keep_naive_matches:
                            ...
                            if translated_answer_num_ocurrences == 1:
                                ...
                                any_answer = True
                                qa['answers'][a_i] = ot_qa[qa['id']]['answers'][a_i]
                        else:
